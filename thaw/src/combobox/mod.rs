...
use self::listbox::{listbox_keyboard_event, Listbox};
use crate::{
...
pub fn Combobox(
    ...
    let options = StoredValue::new(HashMap::<String, types::ComboboxOption>::new());

    let clear_icon_ref = NodeRef::<html::Span>::new();
    let is_show_clear_icon = Memo::new(move |_| clearable && !selected_options.is_empty());
    if clearable {
        Effect::new(move |_| {
                ...
                {if clearable {
                    view! {
                        <span
                            ...
                            style=move || {
                                if !is_show_clear_icon.get() { "display: none" } else { Default::default() }
                            }
